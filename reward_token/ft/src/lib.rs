...
impl Contract {
    ...
    pub fn new(owner_id: AccountId, metadata: FungibleTokenMetadata) -> Self {
        ...
        let mut this = Self {
            ...
            deployed_in_block: env::block_height(),
        };
    ...
    pub fn get_unminted_tokens(&self) -> Balance {
        let diff_block = env::block_height() - self.deployed_in_block;
        diff_block as u128 * NUM_TOKEN_MINT_EVERY_BLOCK - self.token.total_supply
    }

    pub fn mint(&mut self, to: AccountId, amount: Balance) {
        assert!(amount < self.get_unminted_tokens(), "Not enough tokens for reward");
